...
for (const file of files.sort()) {
  ...
  lines.forEach((text, index) => {
    ...
    if (/expect\((?:true|false|\d+|["'][^"']*["'])\)\.toBe\(\1\)/.test(text) || /expect\(\[[^\]]*\]\)\.toContain\([^)]*\)/.test(text)) weak.push({ file, line: index + 1, text: text.trim() });
  });
}
const result = { status: forbidden.length === 0 ? "pass" : "fail", scannedFiles: files.length, forbiddenMarkers: forbidden, weakAssertionWarnings: weak };
console.log(JSON.stringify(result, null, 2));
if (forbidden.length > 0) process.exitCode = 1;
