...
async function ownedTempSet() {
  const { stdout } = await exec("find", [tmpdir(), "-maxdepth", "1", "-type", "d"]);
  return new Set(stdout.split("\n").filter((line) => ownedTempPrefixes.some((prefix) => line.includes("/" + prefix))));
}
async function repoTarballSet() {
  const { stdout } = await exec("find", [root, "-maxdepth", "1", "-type", "f", "-name", "camarade-*.tgz"]);
  return new Set(stdout.split("\n").filter(Boolean));
}
...
async function main() {
  ...
  try {
    ...
    const compiled = await exec(process.execPath, ["dist/src/bin/camarade.js", "--help"], { cwd: root, env: { ...process.env, PATH: "/usr/bin:/bin" } });
    if (!/^Usage: camarade/.test(compiled.stdout) || compiled.stderr) fail("compiled bin help");
    const packed = JSON.parse((await exec("npm", ["pack", "--dry-run", "--json"], { cwd: root })).stdout)[0];
