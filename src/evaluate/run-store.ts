...
export async function listRuns(controllerRoot?:string, onCorrupt?:(entry:string)=>void):Promise<DashboardRunSummary[]> { const dir=root(controllerRoot); const names=await readdir(dir).catch(()=>[] as string[]);const values:DashboardRunSummary[]=[];for(const n of names){const entry=resolve(dir,n),p=resolve(entry,"dashboard-run.json");try{const meta=await lstat(entry);if(!meta.isDirectory()||meta.isSymbolicLink())throw new Error("non-canonical run directory");const x=DashboardRunSchema.parse(JSON.parse(await readFile(p,"utf8")));values.push(DashboardRunSummarySchema.parse({schemaVersion:x.schemaVersion,comparisonId:x.comparisonId,task:x.task,repository:x.repository,timestamps:x.timestamps,status:x.status,outcome:x.outcome,progress:x.progress}))}catch{onCorrupt?.(n)}}values.sort((a,b)=>String(b.timestamps.startedAt).localeCompare(String(a.timestamps.startedAt))||a.comparisonId.localeCompare(b.comparisonId));return DashboardRunListSchema.parse(values)}
