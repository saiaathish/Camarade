...
export function resolveRuleScope(input: ResolveRuleScopeInput): RuleScope {
  ...
  const root = /^(?:AGENTS\.md|CLAUDE\.md|\.github\/copilot-instructions\.md|\.cursor\/rules(?:\/|$))$/i.test(input.sourceRelativePath.replace(/\\/g, "/"));
  const include: string[] = root || /^(?:README\.md|docs(?:\/|$))/i.test(input.sourceRelativePath) ? ["**/*"] : ["**/*"];
  ...
  if (exclusionMatch && !/\bonly\s+for\b/i.test(exclusionMatch[0])) {
    ...
    for (const match of exclusionMatch[1].matchAll(pathPattern)) { const path = cleanPath(match[0]); if (path) exclusions.push(path); }
  }
  pathPattern.lastIndex = 0;
  const included = [...statement.matchAll(pathPattern)].map((m) => cleanPath(m[0])).filter((p): p is string => !!p);
  const specific = included.filter((p) => !exclusions.includes(p) && !technologies.some((tech) => tech.toLowerCase() === p.toLowerCase()));
