...
const sourceKind = (p: string): ContextSourceKind => path.posix.basename(p) === "AGENTS.md" ? "agents" : path.posix.basename(p) === "CLAUDE.md" ? "claude" : p.startsWith(".cursor/rules/") ? "cursor" : p === ".github/copilot-instructions.md" ? "copilot" : p === "README.md" ? "readme" : p.startsWith("docs/") ? "docs" : "configuration";
export async function compileRepositoryIntelligence(input: CompileRepositoryIntelligenceInput): Promise<CompiledRepositoryIntelligence> {
  ...
  if (input.includeGitHistory === false) history = { ...analyzeGitHistory({ records: [] }), availability: "unavailable", degradations: [{ code: "NO_GIT_HISTORY", message: "Git-history analysis was disabled." }], unavailableReason: "Git-history analysis disabled." } as typeof history & { unavailableReason: string };
  else { try { history = analyzeGitHistory(input.historyRecords ? { records: input.historyRecords, ...input.gitHistoryOptions } : { repositoryPath: input.repositoryPath, ...input.gitHistoryOptions }); } catch (error) { if (error instanceof Error && error.message === "repositoryPath must be a Git repository") history = { ...analyzeGitHistory({ records: [] }), availability: "unavailable", degradations: [{ code: "NO_GIT_HISTORY", message: "Repository is not a Git repository." }], unavailableReason: "Git-history analysis unavailable: repository is not a Git repository." } as typeof history & { unavailableReason: string }; else throw error; } }
  ...
  const confidenceAssessments = scoreFindingConfidence({ findings, rules: parsed.rules, evidence: parsed.evidence, facts: inventory.facts, sources: sourceRecords, references: refs.references, architectureDecisions: conventions.architectureDecisions, history, exceptions: exceptions.exceptions });
