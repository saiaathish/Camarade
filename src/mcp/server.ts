...
export function createCamaradeMcpServer(options: { compiler?: ContextCompiler } = {}): McpServer { const server = new McpServer({ name: CAMARADE_MCP_SERVER_NAME, version: CAMARADE_MCP_SERVER_VERSION }); server.registerTool(COMPILE_TASK_CONTEXT_TOOL_NAME, { description: "Compile bounded repository context for a coding task. Requires repository_root and task. It does not execute implementation or validation commands.", inputSchema: compileTaskContextSchema.shape }, (input) => handleCompileTaskContext(input, options.compiler ?? compileContextPipeline)); return server; }
