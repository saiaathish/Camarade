...
export interface RunConditionValidationsInput { condition: ExecutedCondition; preparedCondition: PreparedConditionManifest; commands: readonly string[]; timeoutSeconds: number; conditionDirectory: string; environment: NodeJS.ProcessEnv; environmentEvidence: ValidationEnvironmentEvidence; }
...
export async function runConditionValidations(input: RunConditionValidationsInput): Promise<ConditionValidationResult> {
  ...
  if (input.commands.length === 0 || input.commands.some((c) => c.trim() === "")) throw new Error("Validation command list must be non-empty.");
