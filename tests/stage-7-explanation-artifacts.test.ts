...
describe("S7-03 explanation artifacts", () => {
  ...
  it("[R07] safe run-relative refs", async () => { const f = await written(); const a = await validateExplanationArtifacts(f.root, "cmp-1"); const { directory: _privateDirectory, ...publicArtifacts } = a; expect(JSON.stringify(publicArtifacts)).not.toMatch(/\/Users\/|\/private\/|\/tmp\//); for (const e of a.result.records.flatMap(r => r.evidence)) expect(e.sourceRef).not.toMatch(/^\//); });
