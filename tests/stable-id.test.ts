...
describe("stable IDs", () => {
  ...
  it("accepts new file and fact prefixes with exact format", () => { expect(createStableId("file", ["src/a.ts"])).toMatch(/^file_[0-9a-f]{12}$/); expect(createStableId("fact", ["src/a.ts", 1])).toMatch(/^fact_[0-9a-f]{12}$/); });
  it("preserves existing prefixes and serializes strength", () => { expect(createStableId("source", ["x"])).toMatch(/^source_[0-9a-f]{12}$/); expect(serializeEvidenceGraph(graph())).toContain('"strength": "required"'); });
