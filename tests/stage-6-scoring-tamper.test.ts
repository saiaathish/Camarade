...
describe("S6-R2 persisted tamper proof",()=>{
 ...
 it("[T29] rejects symlink ancestor",async()=>{const r=await fixture();const d=await mkdtemp(join(tmpdir(),"outside-"));await writeFile(join(d,"baseline-score.json"),"x");await writeFile(join(r,"scoring/evidence-index.json"),"{}\n");await rejects(r,SCORING_ERROR_CODES.index)});
