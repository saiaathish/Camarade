...
const artifactGraph = (): IntelligenceArtifactInput["graph"] => ({
  id: "graph_fixture",
  nodes: [{ id: "source_b", kind: "source", label: "src/b.ts" }],
  edges: [],
  danglingReferences: [{ ownerId: "source_b", relation: "evidenceId", missingId: "evidence_missing" }],
});
...
describe("intelligence artifact", () => {
  ...
  it("REQ-ART-17 includes the actual graph content in artifact identity", () => { const original=input(); const changed=input(); changed.graph.nodes[0].label="changed label"; expect(buildIntelligenceArtifact(original).id).not.toBe(buildIntelligenceArtifact(changed).id); });
  it("REQ-ART-18 rejects structurally invalid graph edges", () => { const invalid=input(); invalid.graph.edges.push({ id: "edge_broken", kind: "supports", fromId: "missing", toId: "source_b", explanation: "Broken endpoint." }); expect(() => buildIntelligenceArtifact(invalid)).toThrow("Artifact graph is structurally invalid"); });
  it("REQ-ART-19 refuses to serialize a graph corrupted after artifact construction", () => { const invalid=build(); invalid.graph.nodes.push({ id: "source_b", kind: "source", label: "duplicate" }); expect(() => serializeIntelligenceArtifact(invalid)).toThrow("Artifact graph is structurally invalid"); });
});
