...
describe("loadRunConfig", () => {
  ...
  it("loads the optional Stage 4 context budget without changing legacy defaults", async () => {
    const path = await repo([
      ...
      "    maximum: 9000",
      "    maximum_items: 20",
      "    maximum_evidence_items_per_rule: 2",
      ""
    ].join("\n"));
    expect(await loadRunConfig(path)).toMatchObject({
      validationCommands: [],
      timeoutSeconds: 1800,
  ...
  it.each([
    "context_compiler: nope\n",
    "context_compiler:\n  budget: nope\n",
    "context_compiler:\n  budget:\n    unit: estimated-tokens\n",
    "context_compiler:\n  budget:\n    maximum: 0\n",
    "context_compiler:\n  budget:\n    maximum_items: 1.5\n",
    "context_compiler:\n  budget:\n    maximum_evidence_items_per_rule: -1\n"
  ])("rejects malformed Stage 4 context budget: %s", async (config) => rejectsConfig(await repo(config)));
