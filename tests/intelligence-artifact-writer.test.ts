...
describe("intelligence artifact writer", () => {
  it("REQ-WRITE-01 writes to the default repository-relative artifact path", async () => { const root = await mkdtemp(join(tmpdir(), "camarade-w-")); roots.push(root); const result = await writeIntelligenceArtifact({ repositoryPath: root, artifact }); expect(result.relativePath).toBe(DEFAULT_INTELLIGENCE_ARTIFACT_PATH); expect(result.absolutePath.replaceAll("\\", "/").endsWith(`/${DEFAULT_INTELLIGENCE_ARTIFACT_PATH}`)).toBe(true); });
